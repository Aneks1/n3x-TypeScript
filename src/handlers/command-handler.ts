...
class Command {
    ...
    public run() {
        client.on('messageCreate', async (message: Message) => {
            ...
            if(!name!.startsWith(prefix + this.commandOptions.name)) return
            ...
            if(this.commandOptions.guildCommand == true && !message.guild) { message.reply({ embeds: [new MessageEmbed().setTitle(error + ' Error').setDescription('You can only use this command in a guild.').setColor('#ff2d2d')]} ); return }
            if(this.commandOptions.guildCommand == false && message.guild) { message.reply({embeds: [new MessageEmbed().setTitle(error + ' Error').setDescription("You can't use this command in a guild, try to DM it instead.").setColor('#ff2d2d')] }); return }
            ...
            for (const permission of this.commandOptions.permissions) {
                if(this.commandOptions.permissions[0] != '') {
                    if (!message.member?.permissions.has(permission as PermissionResolvable)) { message.reply({embeds: [new MessageEmbed().setTitle(error + ' Error').setDescription("You don't have enough permissions to use this command.").setColor('#ff2d2d')]}); return }
                }
            }

            for (const requiredRole of this.commandOptions.requiredRoles) {
                if(this.commandOptions.requiredRoles[0] != '') {
                    if (!message.member?.roles.cache.get(requiredRole)) { message.reply({ embeds: [new MessageEmbed().setTitle(error + ' Error').setDescription("You don't have the required roles to use this command.").setColor('#ff2d2d')]}); return }
                }
            }

            for(const allowedChannel of this.commandOptions.allowedChannels) {
                if(this.commandOptions.allowedChannels[0] != ''){
                    if(this.commandOptions.allowedChannels && message.channel.id != allowedChannel) { message.reply({embeds: [new MessageEmbed().setTitle(error + ' Error').setDescription(`This is not an allowed channel to use this command, please go to <#${allowedChannel}>`).setColor('#ff2d2d')]}); return }    
                }
            }

            if(this.commandOptions.maxArgs != undefined) { if(args.length > this.commandOptions.maxArgs || args.length < this.commandOptions.minArgs) { message.reply({embeds: [new MessageEmbed().setTitle(error + ' Error').setDescription(`Syntax error, please use ${name} ${this.commandOptions.expectedArgs}`).setColor('#ff2d2d')]}); return }}
